...
/*   Updated: 2024/03/22 18:36:48 by sbouheni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_image(t_context *context)
{
	...
	image = mlx_new_image(context->mlx_session->mlx, WINDOW_WIDTH,
			WINDOW_HEIGHT);
	if (image == NULL)
...
static void	render_image(t_context *context)
{
	...
	while (++j < WINDOW_HEIGHT)
	{
		...
		while (++i < WINDOW_WIDTH)
		{
			...
			intersection(*context->camera->position, &pixel,
				context->element_list);
			pixel.color_ambient = calculate_ambient_light_color(pixel.col_obj,
					color_to_int(*context->ambient_light->color),
					context->ambient_light->intensity);
			pixel.diffuse_light = search_diffuse_light(pixel, context);
			color = sum_light(pixel.color_ambient, pixel.diffuse_light);
...
int	run_rendering(t_context *context)
{
	...
	init_projection(context->base_cam, context->camera, context->dist_cam,
		context);
